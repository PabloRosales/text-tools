...
const Button = ({ text, disabled, className, onClick, icon, color, small, smaller }: Props) => {
  ...
  if (disabled) {
    col = 'text-slate-500 bg-slate-300 hover:bg-slate-300';
  }
  ...
  return (
    <button
      ...
      } border border-transparent font-medium rounded-md shadow-sm ${col} focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-slate-400 dark:focus:ring-slate-600 ${className}`}
    >
